...
export default function SelectInterest({ background, title, isSelected = false, onSelectChange, ...props }: SelectInterestProps) {
    ...
    }} type="button" className={cn('px-6 py-2 border-3  text-white placeholder-white bg-cover  bg-no-repeat w-full flex flex-row gap-4 cursor-pointer hover:border-white transition-all hover:[&_p]:text-white hover:[&_div]:border-white [&_div]:transition-all', selected ? "border-white [&_div]:bg-white [&_div]:border-white bg-blend-darken " : "border-[#C2C2C2]")} onClick={() => {
